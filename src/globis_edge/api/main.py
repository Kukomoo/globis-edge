...
# ---------------------------------------------------------------------------
# Captive portal catch-all — registered LAST so every real API route above
# takes priority.  When dnsmasq resolves every domain to the Pi, OS probes
# land here with arbitrary paths (e.g. GET / on captive.apple.com).
# We redirect them to /app so the OS captive-portal popup opens the intake wizard.
# ---------------------------------------------------------------------------
...
def captive_catch_all(unknown_path: str, request: Request):
    """Catch-all for captive portal OS probes — redirect to /app."""
    from fastapi.responses import RedirectResponse, JSONResponse
    # Pass through paths that belong to the app or API so 404s surface normally.
    skip_prefixes = (
        "app", "api", "health", "dashboard", "synthesise", "generate",
        "translate", "commit", "audit", "schema", "dignity", "new-session",
        "intake", "quarantine", "demo", "assets", "docs", "openapi",
    )
